function WhiteBlack({ data }: { data: any }) {
  const [TPs, FPs, FNs, TNs] = [
    ...
    <div className={`flex w-full justify-center `}>
      <table className={`border-2`}>
        <thead className={`border-2`}>
          <tr>
            <th>
              {(((TPs + TNs) / (TPs + TNs + FPs + FNs)) * 100).toFixed(2)}%
            </th>
...
function BishopMove({ data }: { data: any }) {
  const [TPs, FPs, FNs, TNs] = [
    data.columnMap.IsDiagonal.items.filter((i: any) => i.role === "IsDiagonal")
      .length,
    data.columnMap.IsDiagonal.items.filter(
      (i: any) => i.role === "IsNotDiagonal"
    ).length,
    data.columnMap.IsNotDiagonal.items.filter(
      (i: any) => i.role === "IsDiagonal"
    ).length,
    data.columnMap.IsNotDiagonal.items.filter(
      (i: any) => i.role === "IsNotDiagonal"
    ).length,
...
function KnightLegal({ data }: { data: any }) {
  const [countCorrect, countIncorrect] = [
