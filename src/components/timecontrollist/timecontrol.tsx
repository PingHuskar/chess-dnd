...
import { getTaskData, isTaskData, type TTask } from './timecontrol-data';
...
export function Task({ task }: { readonly task: TTask }) {
    ...
    return (
        <>
            ...
            {state.type === 'preview' ? createPortal(<DragPreview timecontrol={task} />, state.container) : null}
        </>
    );
}

// A simplified version of our task for the user to drag around
function DragPreview({ timecontrol }: { readonly timecontrol: TTask }) {
    return <div className="border-solid rounded p-2 bg-white">{timecontrol.content}</div>;
}
